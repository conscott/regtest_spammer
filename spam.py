...
SIZE_OF_1_TO_1_TX = round(guess_sz(1, 1) / COIN, 8)
# Want to pay 1 sat/vbyte
DESIRED_FEE_PER_BYTE = 1
# STD FEE
DEFAULT_FEE = SIZE_OF_1_TO_1_TX * DESIRED_FEE_PER_BYTE
...
def wait_for_confirmation(txs_to_confirm=1):
    while len(rpc.listunspent()) < txs_to_confirm:
        miner.generate(1)
        print("Waiting for confirmation of %s transactions..." % txs_to_confirm)
        time.sleep(1)
...
def decider():
    ...
    TOTAL_POSSIBLE_OUTPUTS = int(confirmed_balance / TX_CHAIN_COST)
    num_txs = int(MAX_BLOCK / SPAM_SIZE_PER_OUTPUT_SET) + 1
    amt_per_output = float(round(confirmed_balance / num_txs / MAX_OUTPUTS, 8))
    ...
    if amt_per_output < TX_CHAIN_COST:
        num_txs = int(TOTAL_POSSIBLE_OUTPUTS / MAX_OUTPUTS) + 1
        num_outputs_per_tx = float(round(confirmed_balance / num_txs / TOTAL_POSSIBLE_OUTPUTS, 8))
        amt_per_output = float(round(confirmed_balance / num_txs / num_outputs_per_tx, 8))
    return num_txs, num_outputs_per_tx, amt_per_output


# Make single transaction splitting entire wallet balance between many outputs
def create_many_utxos(at_least_a_block=False):
    num_txs, num_outputs_per_tx, amt_per_output = decider()
    print("Making %s transactions with %s outputs with %s btc" % (num_txs, num_outputs_per_tx, amt_per_output))
    ...
    for i in range(num_txs):
        ...
        print("Transaction %s has txid %s" % (i, txid))

    wait_for_confirmation(txs_to_confirm=num_txs)
...
def make_spending_chain(utxo):
    ...
    for i in range(DEFAULT_ANCESTOR_LIMIT):
        to_send = float(round(utxo['amount'] - decimal.Decimal(DEFAULT_FEE), 8))
        ...
        utxo = {'txid': txid, 'vout': 0, 'amount': decimal.Decimal(to_send)}
